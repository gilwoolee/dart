...
namespace optimizer {
	...
	ForceTorqueConstraint::ForceTorqueConstraint(std::vector<Var *>& _var, Eigen::MatrixXd _A, Eigen::VectorXd _b, int _equality)
	...
	VectorXd ForceTorqueConstraint::evalCon() {
	...
	void ForceTorqueConstraint::fillJac(VVD jEntry, VVB jMap, int index) {
		double w = 1.0;
	...
	void ForceTorqueConstraint::fillObjGrad(std::vector<double>& dG) {
		double w = 1.0;
		VectorXd dP = evalCon();
		for (int j = 0; j < mVariables.size(); ++j)	{
			VectorXd J = mA.col(j);
