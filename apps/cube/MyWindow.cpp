...
void MyWindow::timeStepping()
{
	...
	if (contactEdgeIndex == mRollNum%mEdges.size()) {
		Eigen::Vector3d contactPos = mWorld->getSkeleton(1)->getNode(0)->evalWorldPos(mEdges[contactEdgeIndex]); // contact edge position in world coordinate
		Vector3d contactLocalPos = dart_math::xformHom(mWorld->getSkeleton(0)->getNode(0)->getWorldInvTransform(), contactPos);
		if (mWorld->getSkeleton(1)->getWorldCOM()(0)-contactPos(0) < -0.0) {	
			if (mRollNum < mN-1) {
			  Eigen::Vector3d liftEdge = mWorld->getSkeleton(1)->getNode(0)->evalWorldPos(mEdges[(mRollNum+mEdges.size()-1)%mEdges.size()]);
			  Eigen::Vector3d dropEdge = mWorld->getSkeleton(1)->getNode(0)->evalWorldPos(mEdges[(mRollNum+mEdges.size()+1)%mEdges.size()]);
			  Eigen::Vector3d contactEdge = mWorld->getSkeleton(1)->getNode(0)->evalWorldPos(mEdges[(mRollNum)%mEdges.size()]);
			  double liftAngle = 0.0;
			  liftAngle = atan((liftEdge(1)-contactEdge(1))/(liftEdge(0)-contactEdge(0)));
			  double dropAngle = 0.0;
			  dropAngle = atan((dropEdge(1)-contactEdge(1))/(contactEdge(0)-dropEdge(0)));
...
{
	Eigen::VectorXd pose = mWorld->getSkeleton(1)->getPose();
    switch(key){
	...
	case '=': // increase DoF value
		pose(0) = pose(0) + 0.1;
		mWorld->getSkeleton(1)->setPose(pose,true,true);
		std::cout << mWorld->getSkeleton(1)->getNode(0)->getWorldTransform() << std::endl;
...
void MyWindow::evalInertia()
{
  ...
  double I = mWorld->getSkeleton(1)->getNode(0)->getMass()*numerator/denominator/6.0;
  
  // the order is determined by the order of pivoting
  for (int i = 0; i < numEdges; ++i) {
    mIs[i] = mWorld->getSkeleton(1)->getNode(0)->getLocalInertia() - mWorld->getSkeleton(1)->getNode(0)->getMass()*dart_math::makeSkewSymmetric(mCOM-mEdges[i])*dart_math::makeSkewSymmetric(mCOM-mEdges[i]);
  }
}
...
void MyWindow::evalAngles()
{
  ...
  //double p = 0.5;
  for (int i = 0; i < mN; ++i) {
	  int curIndex = i%mEdges.size(); // current geometry index
	  int nextIndex = (i+1)%mEdges.size(); // next geometry index
	  double m = mWorld->getSkeleton(1)->getNode(0)->getMass();
...
{
	Eigen::Matrix4d preWorldTransformation = mWorld->getSkeleton(0)->getNode(0)->getWorldTransform();
	...
	VectorXd pose = mWorld->getSkeleton(0)->getPose();
	pose(5) = _angle;
	mWorld->getSkeleton(0)->setPose(pose);
	Eigen::Matrix4d curWorldTransformation = mWorld->getSkeleton(0)->getNode(0)->getWorldTransform();
...
int MyWindow::evalContactEdge()
{
	...
	for (int i = 0; i < mWorld->getCollisionHandle()->getNumContacts(); ++i) {
		Eigen::Vector3d contactPos = mWorld->getCollisionHandle()->getCollisionChecker()->getContact(i).point;
		Vector3d contactLocalPos = dart_math::xformHom(mWorld->getSkeleton(1)->getNode(0)->getWorldInvTransform(), contactPos);
