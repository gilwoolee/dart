...
void Controller::evalTargetPose(const Eigen::VectorXd& _dof, const Eigen::VectorXd& _dofVel, const Eigen::VectorXd& _objDof, const Eigen::VectorXd& _objVel, const Eigen::Vector3d& _offset, const Eigen::Vector3d& _targetOri, const Eigen::Vector3d& _objOri) 
{
...
void Controller::computeTorques(const VectorXd& _dof, const VectorXd& _dofVel, const VectorXd& _dofAcc, const VectorXd& _objDof, const VectorXd& _objVel, std::vector<Vector3d>& _contactPoints, std::vector<Vector3d>& _contactForces, std::vector<int>& _contactIndices, const Vector3d& _targetOri, const Vector3d& _objOri) {
	...
	// target
	evalTargetPose(_dof,_dofVel,_objDof,_objVel,onHandOffset,_targetOri,_objOri);
// 	evalTargetOri(_dof,_dofVel,_objDof,_objVel,onHandOffset,_targetOri,_objOri);
	...
	// evaluate force
	if (mControlFlag) {
		evalObjControlForce(_contactPoints,_contactForces,_contactIndices);
	}
	else {
	...
	for (int i = 0; i < mFingerNum; ++i) {
		if (mContactFingers[i]&&!mInContactFingers[i]&&mControlFlag&&!mTrackFingers[i]) {
			for (int j = 0; j < mFingerDofs[i].size(); ++j) {
...
void Controller::evalGravityCompensationForce()
{
	...
	mGravityCompensationForce = mSkel->getExternalForces();
	mSkel->clearExternalForces();
	mSkel->setExternalForces(tempExtForce);
}

void Controller::evalObjControlForce(const std::vector<Eigen::Vector3d>& _contactPoints, const std::vector<Eigen::Vector3d>& _contactForces, const std::vector<int>& _contactIndices)
{
	Eigen::VectorXd tempExtForce = mSkel->getExternalForces();
	mSkel->clearExternalForces();
	...
	mCombinedJTrans = mCombinedJ.transpose(); // the transpose of combined general inverse
...
void Controller::evalTrackForce(const Eigen::VectorXd& _dof, const Eigen::VectorXd& _dofVel)
{
	// other force
	Eigen::VectorXd otherForce;
	otherForce = mSkel->getExternalForces();
	otherForce += mGravityCompensationForce+mObjControlForce+mOriForce+mDampForce+mMaintainForce+mConstraintForce;
...
void Controller::evalTaskForce()
{
	...
	std::vector<Eigen::VectorXd> torqueStars;   // the vector stores the torque for each task
	std::vector<double> inconsistencies;	    // the vector stores the inconsistency for each task	
	...
	while (running) {
		running = false;
		if (numRemainTask == 1) { // only one task
			...
			for (int i = 0; i < numRemainTask; ++i) {
				...
				if (nullSpaceMatrix.cols() >= 2){ // if other tasks have intersection
...
void Controller::evalOriForce(const Eigen::VectorXd& _dof, const Eigen::VectorXd& _dofVel)
{
	...
	otherForce = mSkel->getExternalForces();
	otherForce += mGravityCompensationForce+mObjControlForce+mTrackForce+mDampForce+mMaintainForce+mConstraintForce;
	trackWristOri->updateTask(state,Eigen::Vector3d(angleX,angleY,angleZ).normalized(),otherForce);

	if (dart_math::isZero((mPreOriTarget-trackWristOri->getTarget()).norm())) {
		mAccumulateOriError = mAccumulateOriError + trackWristOri->evalTaskError()*mTimestep;
	}
	else {
